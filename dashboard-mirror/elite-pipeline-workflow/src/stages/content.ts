...
- Blog articles (pageType "blog-article") are subject to a HARD RULE: a MINIMUM of 500 words of body prose; AT LEAST 3 internal links to other pages woven naturally into the prose ([text](/slug)); and AT LEAST 1 outbound link to a high-authority external source (Wikipedia, government/institutional, established trade publication — never a competitor). Use clear <h2>-style section headings (## in markdown).`;
}

function pageUserPrompt(page: PageBlueprint): string {
  // Blog articles have a hard 500-word floor (see brandSystemPrompt).
  const targetWords =
    page.type === "blog-article" ? Math.max(500, page.targetWords) : page.targetWords;
